...
function buildOptionsScript(locs: Location[]): string {
  ...
  const suffix =
    locs.length > 5
      ? ` Showing the top ${top.length} of ${locs.length} results.`
      : "";
...
export const KioskScreen: React.FC<KioskScreenProps> = ({
  ...
  const [savedLocationIds, setSavedLocationIds] = useState<string[]>(() => {
    try {
      const savedJson = localStorage.getItem("saved_locations");
      if (!savedJson) return [];
  ...
  const { playTap, playVoiceStart, playVoiceStop, playSuccess } = soundEffects;
  const speechSynthesis = useSpeechSynthesis({ rate: 0.92 });
  const { speak, cancel: cancelSpeech } = speechSynthesis;
    ...
    (transcript: string, isFinal: boolean) => {
      ...
      setTimeout(() => {
        ...
        if (mode === "post-detail") {
          ...
          if (
            /\b(finish|done|stop|exit|quit|no|goodbye|bye|new search|search again)\b/.test(
              t,
            )
          ) {
            ...
            const prompt =
              "What would you like to explore? Say a category like food, attractions, or transport, or describe what you're looking for.";
            setVoiceState("speaking");
          ...
          const matched = matchLocationChoice(
            transcript,
          ...
          // Could not match
          const retry =
            "Sorry, I didn't catch that. Say a number or name from the list, or say finish to start over.";
          setVoiceState("speaking");
        ...
        if (mode === "choosing") {
          ...
          const retry =
            "Sorry, I didn't catch that. Please say the name or number of your choice.";
          setVoiceState("speaking");
          setVoiceMessage(retry);
          speak(retry, {
            onEnd: () => {
        ...
        if (action.type === "SELECT_LOCATION")
          setSelectedLocation(action.payload as Location | null);
        if (action.type === "SET_VOICE_MESSAGE")
          setVoiceMessage(action.payload as string);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const greeting = "I'm listening. What would you like to explore?";
      setVoiceState("speaking");
  ...
  const featuredLocations = useMemo(() => {
    return filteredLocations.filter(
      (l) => l.featured || savedLocationIds.includes(l.id),
    );
  ...
  const activeCategoryMeta = CATEGORY_META[activeCategory];
                ...
                {activeCategory !== "All" && filteredLocations.length > 0 && (
                  <section>
                    <div className={cn("flex items-center gap-2 mb-3")}>
                      <span className="text-lg">{activeCategoryMeta.icon}</span>
                      <h3 className="text-white/70 text-sm font-bold uppercase tracking-widest">
                        {filteredLocations.length} {activeCategory}{" "}
